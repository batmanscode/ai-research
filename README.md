...
| Graph coefficient continuation | The extremal value is exactly four when `alpha=3`, including an infinite connected family at ratio `4/3`. The `gamma_s <= alpha+1` candidate is now proved for cut-vertex graphs, dominating pairs, dominating induced `P3`s, and all graphs with a dominating triangle. | Active research, not a claimed all-orders solution. By the canonical `P5`-free structure theorem, only inclusion-minimal dominating cliques of order at least four remain. | [`graph/secure-domination-optimal-coefficient/`](graph/secure-domination-optimal-coefficient/) |
