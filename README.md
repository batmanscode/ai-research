...
| Graph coefficient continuation | The extremal value is exactly four when `alpha=3`, including an infinite connected family at ratio `4/3`. Every induced-`P5`-free cut-vertex graph now satisfies `gamma_s <= alpha+1`; exact SAT also finds no `alpha=4` counterexample through order 17. | Active research, not a claimed all-orders solution. The cut-vertex branch and dominating-pair branch are theorems; every remaining counterexample must be 2-connected and have domination number at least three. | [`graph/secure-domination-optimal-coefficient/`](graph/secure-domination-optimal-coefficient/) |
