...
| Graph coefficient continuation | The extremal value is exactly four when `alpha=3`, including an infinite connected family at ratio `4/3`. The `gamma_s <= alpha+1` candidate is now proved for cut-vertex graphs, dominating pairs, and the entire dominating-induced-`P3` branch. | Active research, not a claimed all-orders solution. By the canonical `P5`-free structure theorem, only the dominating-clique branch of order at least three remains. | [`graph/secure-domination-optimal-coefficient/`](graph/secure-domination-optimal-coefficient/) |
