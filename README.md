...
| Graph coefficient continuation | The extremal value is exactly four when `alpha=3`, including an infinite connected family at ratio `4/3`. The `gamma_s <= alpha+1` candidate is proved for cut vertices, dominating pairs and paths, all dominating triangles, and every larger-clique branch with pairwise-anticomplete private regions. | Active research, not a claimed all-orders solution. The remaining core is the multi-part private cross-edge exchange for an inclusion-minimal dominating clique of order at least four. | [`graph/secure-domination-optimal-coefficient/`](graph/secure-domination-optimal-coefficient/) |
